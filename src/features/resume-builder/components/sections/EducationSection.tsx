...
export function EducationSection({ data, onChange }: Props) {
  function add() {
    onChange([...data, { id: generateId(), institution: "", degree: "", field: "", startDate: "", endDate: "", cgpa: "" }]);
  }
      ...
      {data.map((item) => (
        <div key={item.id} className="border rounded-lg p-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Input label="Degree" value={item.degree} onChange={(e) => update(item.id, "degree", e.target.value)} />
            <Input label="Field of Study" value={item.field} onChange={(e) => update(item.id, "field", e.target.value)} />
            <Input label="CGPA" value={item.cgpa} onChange={(e) => update(item.id, "cgpa", e.target.value)} />
