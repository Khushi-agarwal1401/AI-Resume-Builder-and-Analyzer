"use client";

import { useState } from "react";
import { callAi } from "@/features/ai-assistant/api/ai";
import type { ResumeData } from "@/types/resume";
import { cn, generateId } from "@/lib/utils";
import { cleanResumeRewrite } from "@/features/ai-assistant/lib/cleanRewrite";
import { buildResumeText } from "./ResumeOptimizer";

interface AtsKeywordOptimizerProps {
  resumeData?: ResumeData | null;
  /** Write the parsed summary back into the resume. */
  onUpdateSummary?: (summary: string) => void;
  /** Write the parsed experience back into the resume. */
  onUpdateExperience?: (experience: ResumeData["experience"]) => void;
  /** Write the parsed skills back into the resume. */
  onUpdateSkills?: (skills: ResumeData["skills"]) => void;
}

/** Shape returned by POST /api/parse-resume-sections. */
interface ParsedSections {
  summary: string;
  experience: {
    company: string;
    role: string;
    location: string;
    startDate: string;
    endDate: string;
    current: boolean;
    responsibilities: string[];
  }[];
  education: { institution: string; degree: string; field: string; startDate: string; endDate: string; cgpa: string }[];
  skills: { technical: string[]; soft: string[]; tools: string[]; frameworks: string[] };
  projects: { name: string; description: string; technologies: string[]; liveUrl: string; githubUrl: string }[];
}

/**
 * Rewrites the resume with the job description's most relevant keywords woven
 * in naturally. Uses only the candidate's actual background — nothing is
 * fabricated (the action's critical rules enforce this). The output is
 * editable, and Apply parses it back into the resume's structured sections.
 */
export function AtsKeywordOptimizer({
  resumeData,
  onUpdateSummary,
  onUpdateExperience,
  onUpdateSkills,
}: AtsKeywordOptimizerProps) {
  const [targetRole, setTargetRole] = useState("");
  const [jobDescription, setJobDescription] = useState("");
  const [result, setResult] = useState<string | null>(null);
  const [editedText, setEditedText] = useState("");
  const [loading, setLoading] = useState(false);
  const [applying, setApplying] = useState(false);
  const [applied, setApplied] = useState(false);
  const [applyMsg, setApplyMsg] = useState<string | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [copied, setCopied] = useState(false);

  const resumeText = resumeData ? buildResumeText(resumeData) : "";

  const canRun = Boolean(resumeText.trim() || targetRole.trim() || jobDescription.trim());

  async function handleOptimize() {
    if (!canRun) return;

    const inputParts = [targetRole.trim() ? `Target role: ${targetRole.trim()}` : "", jobDescription.trim()]
      .filter(Boolean)
      .join("\n\n");
    const input = inputParts || "No target role or job description provided — optimize keywords for general best practices.";

    setLoading(true);
    setError(null);
    setResult(null);
    setApplied(false);
    setApplyMsg(null);

    try {
      const res = await callAi("ats-keyword-optimization", input, resumeText);
      if (res.success) {
        // The model occasionally appends a "Note: ..." explanation after the
        // resume — strip it (plus any leading preamble) so only the resume remains.
        const cleaned = cleanResumeRewrite(res.output);
        setResult(cleaned);
        setEditedText(cleaned);
      } else {
        setError(res.error || "Failed to optimize keywords");
      }
    } catch {
      setError("Something went wrong. Please try again.");
    } finally {
      setLoading(false);
    }
  }

  function handleCopy() {
    if (!editedText) return;
    navigator.clipboard.writeText(editedText);
    setCopied(true);
    setTimeout(() => setCopied(false), 2000);
  }

  /** Parse the edited text and write the parsed sections back into the resume. */
  async function handleApply() {
    if (!editedText.trim() || applying) return;
    setApplying(true);
    setApplyMsg(null);
    try {
      const res = await fetch("/api/parse-resume-sections", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ text: editedText }),
      });
      const json = await res.json();
      if (!json.success || !json.data) {
        setApplyMsg(json.error || "Could not parse the edited resume.");
        return;
      }
      const parsed = json.data as ParsedSections;

      const updates: string[] = [];

      // Summary
      if (parsed.summary) {
        onUpdateSummary?.(parsed.summary);
        updates.push("Summary");
      }

      // Experience — preserve ids/achievements for entries that still exist,
      // generate ids for new ones so the builder stays consistent.
      if (parsed.experience.length > 0 && onUpdateExperience) {
        const existing = resumeData?.experience ?? [];
        const mapped = parsed.experience.map((e) => {
          const prev = existing.find(
            (x) =>
              x.company.toLowerCase() === e.company.toLowerCase() &&
              x.role.toLowerCase() === e.role.toLowerCase()
          );
          return {
            id: prev?.id ?? generateId(),
            company: e.company,
            role: e.role,
            location: e.location,
            startDate: e.startDate,
            endDate: e.endDate,
            current: e.current,
            responsibilities: e.responsibilities,
            achievements: prev?.achievements ?? [],
          };
        });
        onUpdateExperience(mapped);
        updates.push("Experience");
      }

      // Skills
      const skills = parsed.skills;
      const hasSkills =
        skills.technical.length > 0 ||
        skills.soft.length > 0 ||
        skills.tools.length > 0 ||
        skills.frameworks.length > 0;
      if (hasSkills && onUpdateSkills) {
        onUpdateSkills({
          technical: skills.technical,
          soft: skills.soft,
          tools: skills.tools,
          frameworks: skills.frameworks,
        });
        updates.push("Skills");
      }

      if (updates.length === 0) {
        setApplyMsg("No summary, experience, or skills could be extracted from the edited text.");
        return;
      }

      setApplied(true);
      setApplyMsg(`Applied ${updates.join(", ")} to your resume.`);
      setTimeout(() => setApplied(false), 3000);
    } catch {
      setApplyMsg("Something went wrong while applying the changes.");
    } finally {
      setApplying(false);
    }
  }

  return (
    <div className="space-y-4">
      <div>
        <label className="block text-[12px] font-medium text-gray-700 mb-1.5">
          <span className="flex items-center gap-1">
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-gray-400">
              <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/>
              <circle cx="12" cy="7" r="4"/>
            </svg>
            Target Role
          </span>
        </label>
        <input
          className="h-10 w-full rounded-lg border border-gray-200 bg-white px-3 text-[13px] outline-none transition-all duration-200 focus:border-accent-400 focus:ring-[3px] focus:ring-accent-500/15 hover:border-gray-300"
          value={targetRole}
          onChange={(e) => setTargetRole(e.target.value)}
          placeholder="e.g. Senior Full Stack Engineer"
        />
      </div>

      <div>
        <label className="block text-[12px] font-medium text-gray-700 mb-1.5">
          <span className="flex items-center gap-1">
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-gray-400">
              <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/>
              <path d="M14 2v6h6"/>
              <path d="M16 13H8M16 17H8M10 9H8"/>
            </svg>
            Job Description <span className="text-gray-400 font-normal">(optional but recommended)</span>
          </span>
        </label>
        <textarea
          className="w-full h-24 rounded-lg border border-gray-200 bg-white px-3 py-2.5 text-[13px] outline-none transition-all duration-200 focus:border-accent-400 focus:ring-[3px] focus:ring-accent-500/15 hover:border-gray-300 resize-y placeholder:text-gray-300"
          value={jobDescription}
          onChange={(e) => setJobDescription(e.target.value)}
          placeholder="Paste the job description so the most relevant keywords are woven into the resume..."
        />
      </div>

      <button
        onClick={handleOptimize}
        disabled={loading || !canRun}
        className={cn(
          "w-full px-4 py-2.5 rounded-xl text-small font-semibold transition-all duration-200",
          "bg-accent-600 text-white hover:bg-accent-700",
          "disabled:opacity-50 disabled:cursor-not-allowed"
        )}
      >
        {loading ? (
          <span className="flex items-center justify-center gap-2">
            <span className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin" />
            Optimizing keywords...
          </span>
        ) : (
          "Optimize Keywords"
        )}
      </button>

      {error && (
        <div className="p-3 rounded-lg bg-red-50 border border-red-200 flex items-start gap-2.5">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-red-500 mt-0.5 shrink-0">
            <circle cx="12" cy="12" r="10"/>
            <line x1="12" y1="8" x2="12" y2="12"/>
            <line x1="12" y1="16" x2="12.01" y2="16"/>
          </svg>
          <p className="text-[12px] text-red-700">{error}</p>
        </div>
      )}

      {result !== null && (
        <div className="rounded-xl border border-accent-200 bg-gradient-to-br from-accent-50/80 to-white overflow-hidden transition-all duration-300 animate-in fade-in slide-in-from-bottom-2 dark:from-accent-500/10 dark:to-gray-900 dark:border-accent-500/25">
          <div className="flex items-center justify-between px-4 py-3 border-b border-accent-100 bg-accent-50/50">
            <div className="flex items-center gap-2">
              <div className="w-5 h-5 rounded-full bg-accent-100 flex items-center justify-center">
                <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" className="text-accent-600">
                  <polyline points="20 6 9 17 4 12"/>
                </svg>
              </div>
              <span className="text-[12px] font-semibold text-accent-800">Optimized Resume</span>
            </div>
            <div className="flex items-center gap-1.5">
              <button
                onClick={handleCopy}
                className="px-2.5 py-1 rounded-md text-[11px] font-medium text-gray-500 hover:bg-gray-100 transition-colors"
              >
                {copied ? "Copied!" : "Copy"}
              </button>
              <button
                onClick={handleApply}
                disabled={applying || !editedText.trim()}
                className={cn(
                  "px-2.5 py-1 rounded-md text-[11px] font-medium transition-colors",
                  applied
                    ? "bg-green-100 text-green-700"
                    : "text-accent-600 hover:bg-accent-100",
                  "disabled:opacity-50 disabled:cursor-not-allowed"
                )}
              >
                {applying ? "Applying…" : applied ? "Applied!" : "Apply"}
              </button>
            </div>
          </div>
          <div className="p-4">
            <textarea
              value={editedText}
              onChange={(e) => setEditedText(e.target.value)}
              rows={14}
              aria-label="Editable optimized resume"
              className="w-full rounded-lg border border-gray-200 bg-white px-3 py-2.5 text-[13px] text-gray-700 outline-none transition-all duration-200 focus:border-accent-400 focus:ring-[3px] focus:ring-accent-500/15 hover:border-gray-300 resize-y leading-relaxed font-mono"
            />
            {applyMsg && (
              <p className={cn("text-[11px] mt-2", applied ? "text-green-600" : "text-amber-600")}>
                {applyMsg}
              </p>
            )}
          </div>
        </div>
      )}

      <div className="flex items-center gap-2 text-[11px] text-gray-400">
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
          <circle cx="12" cy="12" r="10"/>
          <path d="M12 16v-4M12 8h.01"/>
        </svg>
        <span>Weaves the job description's most relevant keywords into the resume — using only keywords supported by your actual background, never keyword-stuffed. Edit the output, then Apply to write it back into your resume.</span>
      </div>
    </div>
  );
}
