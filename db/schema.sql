...
--
--  Apply it with `npm run db:migrate` (psql -f db/schema.sql) against any
...
--  • New column → add it to the CREATE TABLE body FIRST (so `npm run db:gen-types`
...
--    `npm run db:check-types` fail loudly — see scripts/db-generate-types.mjs.
--
--  Notes:
...
-- existing databases the next time they run `npm run db:migrate`; fresh databases
